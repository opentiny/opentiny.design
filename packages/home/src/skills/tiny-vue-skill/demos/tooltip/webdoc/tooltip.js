export default{column:'2',owner:'',show:true,cloud:true,demos:[{demoId:'basic-usage',name:{'zh-CN':'基本用法','en-US':'Basic Usage'},desc:{'zh-CN':`通过 <code>content</code> 属性指定提示的内容。<br>
           通过 <code>placement</code> 属性指定提示的显示位置，支持 12 个显示位置。<br>
           通过 <code>effect</code> 属性指定提示的效果。<br>`,'en-US':`Specify the content of the prompt via the <code>content</code> attribute <br>
          Use the <code>placement</code> property to specify the display location of the prompt. 12 display locations are supported <br>
          Specify the effect of the prompt with the <code>effect</code> property <br>`},codeFiles:['basic-usage.vue']},{demoId:'theme',name:{'zh-CN':'主题','en-US':'Theme'},desc:{'zh-CN':`
            通过 <code>effect</code> 属性设置明暗效果，可选值 <code>dark/light</code> ,默认值为 <code>dark</code>，通常使用<code>effect</code>设置效果。<br>
            通过 <code>type</code> 属性设置主题，它的优先级大于 <code>effect</code>。`,'en-US':`
            ...
            Set the theme with the <code>type</code> property, which has a higher priority than the <code>effect</code>.`},codeFiles:['theme.vue']},{demoId:'control',name:{'zh-CN':'控制/禁用提示','en-US':'Control/disable'},desc:{'zh-CN':`
          ...
          3.通过 <code>disabled</code> 属性，直接禁用提示。<br>`,'en-US':`
          ...
          3. Use the <code>disabled</code> property to disable the tooltip. <br>`},codeFiles:['control.vue']},{demoId:'content',name:{'zh-CN':'弹出层内容','en-US':'Tooltip content'},desc:{'zh-CN':`
          通过 <code>content</code> 属性指定弹出的文本。<br>
          通过 <code>renderContent</code> 自定义渲染函数，可使用 <code>JSX</code> 返回需要渲染的节点内容。<br>
          通过 <code>content</code> 插槽，自定义内容，当弹出复杂的内容结构时，推荐使用插槽的方式。<br>`,'en-US':`
          ...
          The <code>content</code> slot is used to customize content. It is recommended to use the slot when a complex content structure pops up. <br>`},codeFiles:['content.vue']},{demoId:'offset',name:{'zh-CN':'弹出层的偏移','en-US':'Offset'},desc:{'zh-CN':'通过 <code>offset</code> 属性设置组件出现位置的偏移量\u3002','en-US':'Use the <code>offset</code> property to set the offset of the position where the Tooltip component appears. '},codeFiles:['offset.vue']},{demoId:'custom-popper',name:{'zh-CN':'弹出层的特性','en-US':'Features of the popper layer'},desc:{'zh-CN':`
          通过 <code> visible-arrow </code> 属性设置是否显示小箭头。<br>
          通过 <code> enterable </code> 属性设置鼠标是否可进入到 tooltip 中。<br>
          通过 <code> popper-class </code> 属性设置弹出 dom 的类名，便于用户定制弹出层的样式。<br>`,'en-US':`
          ...
          You can use the <code> popper-class </code> property to set the class name of the eject dom to customize the style of the eject layer. <br>`},codeFiles:['custom-popper.vue']},{demoId:'delay',name:{'zh-CN':'延迟控制','en-US':'Delay control'},desc:{'zh-CN':`
          ...
          `,'en-US':`
            ...
            `},codeFiles:['delay.vue']},{demoId:'popper-options',name:{'zh-CN':'高级配置','en-US':'Advanced configuration'},desc:{'zh-CN':'通过 <code>popper-options</code> 属性为组件的弹出层的配置参\u3002','en-US':'Use the <code>popper-options</code> property to configure popper parameters for Tooltip. '},codeFiles:['popper-options.vue']},{demoId:'transition',name:{'zh-CN':'自定义渐变动画','en-US':'Customized Animation'},desc:{'zh-CN':'通过 <code>transition</code> 定义渐变动画\uFF0C默认选值为 <code>tiny-fade-in-linear</code>\u3002','en-US':'Define a gradient animation using <code>transition</code>. The default value is <code>tiny-fade-in-linear</code>'},codeFiles:['transition.vue']},{demoId:'pre',name:{'zh-CN':'文本预格式化','en-US':'Customized Animation'},desc:{'zh-CN':`
          配置 <code> pre </code>  为  <code> true </code> ，就会预格式化  <code> content </code>  文本。<br>
          被包围在 <code> pre </code>  标签元素中的文本会保留空格和换行符，文本也会呈现为等宽字体。`,'en-US':`
          If <code> pre </code> is set to <code> true </code>, the <code> content </code> text is preformatted.<br>
          Text enclosed in the <code> pre </code> tag element retains spaces and newline characters, and the text is rendered in a uniform font.`},codeFiles:['pre.vue']},{demoId:'content-max-height',name:{'zh-CN':'内容最大高度','en-US':'Content max height'},desc:{'zh-CN':`配置 <code>content-max-height</code>  设置内容展示的最大高度。`,'en-US':`Configure <code> content-max-height </code> to set the maximum height for content display.`},codeFiles:['content-max-height.vue']}],features:[{id:'basic',name:'基本用法',support:{value:true},description:'通过 content 属性设置提示内容\uFF0Cplacement 设置显示位置\u3002',cloud:{value:true},apis:['content','placement'],demos:['basic-usage']},{id:'theme',name:'主题',support:{value:true},description:'通过 effect 设置明暗效果\uFF0Ctype 设置主题类型\u3002',cloud:{value:true},apis:['effect','type'],demos:['theme']},{id:'control',name:'控制显示',support:{value:true},description:'通过 visible 控制显示状态\uFF0Cmanual 设置手动控制模式\u3002',cloud:{value:true},apis:['visible','manual'],demos:['control']},{id:'content',name:'自定义内容',support:{value:true},description:'通过 renderContent 函数或 content 插槽自定义提示内容\u3002',cloud:{value:true},apis:['renderContent','slots'],demos:['content']}]};